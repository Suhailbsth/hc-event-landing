...
export default function SuccessMessage({ registration, event }: SuccessMessageProps) {
  ...
  const registrationId = regData.registrationId || regData.id;
  ...
  const handleAddToAppleWallet = async () => {
    ...
    try {
      ...
      // Must use fetch() with POST, then download the binary blob
      const response = await fetch(
        `${API_BASE_URL}/api/EventWallet/apple/generate-event-ticket?registrationId=${registrationId}&email=${encodeURIComponent(regData.email || '')}`,
        { method: 'POST' }
      );
      ...
      a.href = url;
      a.download = `event-pass-${regData.serialNumber || 'pass'}.pkpass`;
  ...
  const handleAddToGoogleWallet = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/EventWallet/google/generate-event-ticket?registrationId=${registrationId}&email=${encodeURIComponent(regData.email || '')}`,
        { method: 'POST' }
      );
  ...
  const handleAddToSamsungWallet = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/EventWallet/samsung/generate-event-ticket?registrationId=${registrationId}&email=${encodeURIComponent(regData.email || '')}`,
        { method: 'POST' }
      );
